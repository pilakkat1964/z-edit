#!/usr/bin/env python3
"""
z-edit Release Automation Script

Automates the release process for z-edit by updating version numbers across
...
class ReleaseAutomation:
    """Automates the z-edit release process."""
    ...
    def update_cmakelists(self):
        ...
        pattern = r"project\(\s*zedit\s+VERSION\s+[\d\.]+\s+"
        replacement = f"project(zedit\n    VERSION      {self.version}\n    "
    ...
    def update_debian_changelog(self):
        ...
        new_entry = (
            f"zedit ({debian_version}) unstable; urgency=low\n"
            f"\n"
            f"{changelog_msg}"
            f"\n"
    ...
    def git_add_and_commit(self):
        ...
        if not self.dry_run:
            try:
                ...
                self.error(
                    f"Git commit failed: {e.stderr.decode() if e.stderr else str(e)}"
                )
        else:
    ...
    def git_push_branch(self):
        ...
        if not self.dry_run:
            try:
                ...
                self.error(
                    f"Git push failed: {e.stderr.decode() if e.stderr else str(e)}"
                )
        else:
    ...
    def git_create_tag(self):
        ...
        if not self.dry_run:
            try:
                ...
                self.error(
                    f"Git tag creation failed: {e.stderr.decode() if e.stderr else str(e)}"
                )
        else:
    ...
    def git_push_tag(self):
        ...
        if not self.dry_run:
            try:
                ...
                self.error(
                    f"Git tag push failed: {e.stderr.decode() if e.stderr else str(e)}"
                )
        else:
    ...
    def show_summary(self):
        ...
        print(
            f"  2. Download from: https://github.com/pilakkat1964/z-edit/releases/tag/{self.tag}"
        )
        print(f"  3. Install via: sudo apt install ./zedit-{self.version}-Linux.deb")
        print("=" * 60 + "\n")

    def confirm(self):
        """Ask user to confirm release."""
        print("\n" + "=" * 60)
        print(f"Release Confirmation: z-edit {self.version}")
        print("=" * 60)
    ...
    def run(self):
        ...
        self.log("Starting z-edit release automation...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automate z-edit release process",
